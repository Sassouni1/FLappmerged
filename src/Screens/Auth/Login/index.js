...
const Login = ({navigation}) => {
  ...
  const login = async () => {
    ...
    if (!emailError && !passwordError) {
      ...
      dispatch(loginRequest({email: email, password: password,role:'customer'}));
    } else {
  ...
  return (
    <View style={styles.contaner}>
      ...
      <Text style={styles.txt}>DaruStrong</Text>
        ...
        <Text style={styles.txt2}>Welcome,{'\n'}<Text style={{color:'rgba(189, 189, 189, 1)'}}>Login to continue</Text></Text>
          ...
          <KeyboardAwareScrollView
            ...
            <Button
              onPress={() => login()}
              text="Login"
              btnStyle={{...GernalStyle.btn, marginTop: getHeight(5)}}
