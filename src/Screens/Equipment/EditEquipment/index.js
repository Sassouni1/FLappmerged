...
const EditEquipment = ({route}) => {
  ...
  // const Category = ["Egypt", "Canada", "Australia", "Ireland"]
  const [errors, setErrors] = useState({});
 
  const [file, setFile] = useState(data?.images);
  ...
  const save = async () => {
    ...
    const equipmentNameError = await validator('reason', equipmentName);
    ...
    if (
      ...
      specifications.map(item => formData.append('specifications', item));
      formData.append('equipmentName', equipmentName);
  
      fetch(BASE_URL +  `/equipment/updateEquipment/${data?._id}`, {
        method: 'PUT',
        body: formData,
  ...
  const Addspecification = async () => {
    ...
    // setState({...state, specificationsError: ''});
    // setErrors1({...errors1, specifications: ''});
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <View style={{marginBottom: getHeight(2), marginTop: getHeight(2)}}>
          ...
          <TextInput
            ...
            onChangeText={equipmentName =>
              changeHandler('equipmentName', equipmentName.trim())
            }
          />
        </View>
        <SelectDropdown
          ...
          onSelect={(selectedItem, index) => {
            setState({...state, category: selectedItem, IssueError: ''});
          }}
      ...
      <FlatList
        ...
        data={specifications}
        ListFooterComponent={() => (
          <View style={{marginLeft: getWidth(5), marginBottom: getHeight(10)}}>
