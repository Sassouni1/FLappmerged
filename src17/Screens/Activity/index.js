...
export default function TrainingStats({ navigation }) {
  ...
  const [stepCounterData, setStepCounterData] = useState([]);
  const [steps_dropdown, set_steps_dropdown] = useState("Last 7 Days");
    ...
    AppleHealthKit.initHealthKit(options, (err, results) => {
      ...
      AppleHealthKit.getHeartRateVariabilitySamples(
        ...
        (err, results) => {
          ...
          setHealthData((prevData) => ({
            ...prevData,
            heartRateVariability: results?.[0]?.value || "N/A",
          }));
        }
      );
      ...
      AppleHealthKit.getRestingHeartRateSamples(
        ...
        (err, results) => {
          ...
          setHealthData((prevData) => ({
            ...prevData,
            restingHeartRate: results?.[0]?.value || "N/A",
          }));
        }
      );
      ...
      AppleHealthKit.getDistanceWalkingRunning(
        ...
        (err, results) => {
          ...
          setHealthData((prevData) => ({
            ...prevData,
            distanceWalkingRunning: results?.value || "N/A",
          }));
        }
      );
      ...
      AppleHealthKit.getActiveEnergyBurned(
        ...
        (err, results) => {
          ...
          setHealthData((prevData) => ({
            ...prevData,
            activeEnergyBurned: results?.[0]?.value || "N/A",
          }));
        }
      );
      ...
      AppleHealthKit.getDailyStepCountSamples(
        ...
        (err, results) => {
          ...
          const formattedData = results.map((item) => ({
            value: item.value,
            label: new Date(item.startDate).toLocaleDateString("en-US", {
              weekday: "short",
            }),
          }));
          setStepCounterData(formattedData);
        }
      );
      AppleHealthKit.getHeartRateVariabilitySamples(
        { startDate: startDate.toISOString(), endDate: endDate.toISOString() },
  ...
  const onChangeDropDown = (selectedType, section) => {
    ...
    else if (section == "steps") set_steps_dropdown(selectedType);
  ...
  const getWeightProgress = async (timePeriod, setProgressFunction) => {
    ...
    const getStepCountProgress = async (timePeriod, setProgressFunction) => {
      const endDate = new Date();
  ...
  return (
    <ScrollView>
        ...
        <View>
          ...
          <View style={styles.chartOuterContainer}>
              ...
              <View style={styles.headerTextStyle}>
                <Text style={styles.percentageStyle}>
                  {selectedAppleStat === "steps"
                    ? stepCounterData.reduce((sum, day) => sum + day.value, 0)
                    : hrvData.length > 0
                    ? hrvData[hrvData.length - 1].value.toFixed(2)
                    : "N/A"}
                </Text>
            ...
            <LineChart
              areaChart
              curved
              data={selectedAppleStat === "steps" ? stepCounterData : hrvData}
              width={width - getFontSize(30)}
      ...
      <Button
        ...
        onPress={requestPermissionsAndFetchData}
      />
      <View style={{ height: 100 }} />
    </ScrollView>
  );
}
// Memoized Component for Step Counter Chart
const StepCounterComponent = memo(({ stepCounterData }) => {
  return (
    <View>
      <View style={styles.trainingContainerStyle}>
        <Text style={styles.trainingFontStyle}>Daily Step Count</Text>
